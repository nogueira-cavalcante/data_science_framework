...
def bayes_search(y_train, X_train=None, training_type='time_series',
                 info_model_matrix=[['holt_winters',
                                     {'trend': ['add', 'mul']},
                                     2]],
                 n_splits=2, validation_size=0.1,
                 random_state=10):
    """
    ...
    if (validation_size <= 0.0) or (validation_size >= 0.5):
        ...
        train_size = int((X_train.shape[0] / n_splits) * (1 - validation_size))
    ...
    df_best_models.set_index('name_models', inplace=True)
