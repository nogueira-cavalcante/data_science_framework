...
def cv_score(X_train, y_train, estimator, params, scorer, training_type, cv):
    
    import numpy as np
    
    if cv <= 0:
        
        raise Exception('O parâmetro cv deve ser inteiro maior que zero.')
    ...
    if training_type == 'classification':
        ...
        raise Exception('Ainda não implemetando!')
        ...
        cv_list = np.arange(1, cv + 1)
                
        for cv_indice in cv_list:
    
            indices_train = X_train.index[:-cv_indice]        
            X_train_cv_list.append(X_train.loc[indices_train, :])
            y_train_cv_list.append(y_train.loc[indices_train, :])
            
            indice_val = X_train.index[-cv_indice]
            X_val_cv_list.append(X_train.loc[indice_val:indice_val, :])
            y_val_cv_list.append(y_train.loc[indice_val:indice_val, :])
    
    for cv_indice in np.arange(cv):
        
        est = estimator()
        est.set_params(**params)
        est.fit(X_train_cv_list[cv_indice], y_train_cv_list[cv_indice])
        
        scores_list.append(scorer(y_val_cv_list[cv_indice],
                           est.predict(X_val_cv_list[cv_indice])))
        indices_list.append(X_val_cv_list[cv_indice].index.astype(str).all())
        
    return scores_list, indices_list

def optimized_search_cv(X_train, y_train, X_test, y_test, training_type,
                        info_model_matrix, cv=3, random_state=10):
    """
    Otimizador de hiperparâmetros automático.
    ...
    best_fitted_model_list = []
    best_refitted_model_list = []
    best_cv_scores_list = []
    ...
    for i in range(len(models_list)):
        ...
        def objective(**params):
            ...
            # score = scorer(y_train, estimator_fit.predict(X_train))
            
            scores_list, indices_list = cv_score(X_train, y_train, estimator,
                                                 params, scorer, training_type,
                                                 cv)
            
            scores_dict = {indices_list[i]: scores_list[i]
                           for i in range(len(indices_list))}
            scores_cv_list.append(scores_dict)
        ...
        best_fitted_model = model_fitted_list[index_best_model]
        best_refitted_model = model_refitted_list[index_best_model]
        best_cv_scores = scores_cv_list[index_best_model]

        best_fitted_model_list.append(best_fitted_model)
        best_refitted_model_list.append(best_refitted_model)
        best_cv_scores_list.append(best_cv_scores)
        
        score_train = scorer(y_train, best_fitted_model.predict(X_train))
        score_test = scorer(y_test, best_fitted_model.predict(X_test))
    ...
    df_best_models = pd.DataFrame(data=np.array([best_fitted_model_list,
                                                 best_refitted_model_list,
                                                 best_parameters,
                                                 best_cv_scores_list,
                                                 score_train_list,
